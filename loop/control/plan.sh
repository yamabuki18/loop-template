...
have_credential || die "no credential set (control/secret.env): set CLAUDE_CODE_OAUTH_TOKEN (subscription) or ANTHROPIC_API_KEY (metered API)."
...
- /memory/PROGRESS.md  (what already landed / failed — do NOT re-plan finished work)
...
1) /out/slices.json — a JSON array of AT MOST ${PLANNER_MAX_SLICES} slices. Each slice:
   ...
   - Fewer, cleanly-separated slices beat many overlapping ones. 1 slice is fine.
...
docker run --rm \
  ...
  -w /out \
  "$IMAGE" -lc 'claude -p "$PLAN_PROMPT" --dangerously-skip-permissions' 2>&1 \
  | sed 's/^/  [planner] /'
prc=${PIPESTATUS[0]}
set -e
...
jq -e 'type=="array" and length>=1' "$slices" >/dev/null 2>&1 \
  || die "slices.json is not a non-empty JSON array: $slices"
