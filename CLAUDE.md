...
herdr、秘密はスコープ別の平文 env、セカンドオピニオンは codex）。運用者向けドキュメントは
...
loop/
  ...
  control/            エンジン本体（全 bash スクリプト + フック）
    ...
    ontology-check.sh AIF イベントオントロジー（memory/ontology/graph.jsonl）の決定論検証
                      （digest 再生成時=land ごとに自動実行。違反は ONTOLOGY_INVALID 警告のみ）
  ...
  CHANGELOG.md        変更履歴 + 実運用で踏んだ不具合の記録（README から分離。現行仕様は README が正）
```

## テスト（改修したら必ず回す）

```bash
bash loop/tests-toolkit/run.sh          # 数秒（フック契約 + lib 単体。件数は出力の総括行が正）
...
- 秘密は**スコープ別**（worker/gate/codex）の**平文** `secret.<scope>.env`（v3.7 で sops+age を
  撤去。gitignore が唯一の防壁 — `secret.*.env` の ignore 行を消す変更は不可）。値は
  `secret_exec <scope> -- cmd` が**その 1 プロセスの env にだけ**注入する。ループ本体の
  シェルに秘密を source しない（lib.sh は config.env しか source しない）。
- `secret.gate.env` は gate の決定論チェック専用、`secret.codex.env` は codex 専用。
  **どちらも Claude プロセスには決して渡さない。** 新しい注入点を足すときはこの表を壊して
  いないか必ず確認する。
- `secret_present` は「ファイル存在」ではなく「空でない値の代入がある」判定。init/setup が
  空テンプレートを seed するため、存在判定に緩めるとホストログイン・フォールバック
  （claude_cfg_seed）と usage guard のプローブが黙って死ぬ（テストで固定済み）。
